<!-- locale-guard:language-bar:start -->
**<img src="../../../_generated/assets/flags/us.svg" alt="English" width="20" height="auto"> English** | <img src="../../../_generated/assets/flags/es.svg" alt="Español" width="20" height="auto"> Español *(missing)* | <img src="../../../_generated/assets/flags/de.svg" alt="Deutsch" width="20" height="auto"> Deutsch *(missing)* | <img src="../../../_generated/assets/flags/jp.svg" alt="日本語" width="20" height="auto"> 日本語 *(missing)*
<!-- locale-guard:language-bar:end -->

# Testing – Bluewater Framework

📄 **File:** `docs/en/technical/testing/index.md`
📅 **Status:** Active
🏷️ **Tags:** technical, testing, quality
🔖 **Version:** 8.0.0
📅 **Date:** 2026-09-03
🌍 **Scope:** Application and framework verification paths
🤝 **Contributors:** Bluewater framework maintainers
👨‍💻 **Author:** Bluewater Framework Team

---

> ### 🪶 **Bluewater Principle**
> *Tests prove public behavior at the narrowest effective level.*

---

## 📌 Purpose

This index separates application testing from framework-core validation.

## Test paths

- [Application testing](applications.md) covers tests owned by a Bluewater application and the bundled reference app.
- [Framework testing](framework.md) covers subsystem, integration, static-analysis, style, and external-host validation.

## 📚 Related Documents

- [Application testing](applications.md)
- [Framework testing](framework.md)
- [Contribution workflow](../development/contributing.md)

---

This published documentation is licensed under the [MIT License](../../../../LICENSE). Bluewater Framework source code is separately licensed under OSL-3.0.

---

*Last updated: 2026-09-03*
